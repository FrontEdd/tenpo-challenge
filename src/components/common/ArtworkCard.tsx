...
function ArtworkImage({ item, imageClass }: { item: ListItem; imageClass: string }) {
  ...
  return (
    <div className={`relative w-full ${imageClass} overflow-hidden bg-gray-100 flex-shrink-0`}>
...
export function ArtworkCard({ item, imageClass = 'h-48' }: ArtworkCardProps) {
  ...
  return (
    <div className="group bg-white rounded-xl overflow-hidden border border-gray-100 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 flex flex-col h-full">
      <ArtworkImage item={item} imageClass={imageClass} />
